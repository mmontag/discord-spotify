...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('queue')
        .setDescription('Add song to the end of current playback queue.')
    ...
    async execute(interaction) {
        ...
        try {
            ...
            selectedTrack.update({ content: `Queueing ${trackLabel}`, components: [] });
            await spotifyApi.ensureAccessToken();
            // TODO: extract this play fallback behavior to Spotify API wrapper

            console.log(`Queueing ${trackLabel}... with track URI ${trackUri}`);

            const success = await spotifyApi.addToQueue(trackUri)
                .then(data => {
                    console.log(`${trackLabel} queued successfully.`);
                    return true;
                }, async err => {
                    console.log(`Error queuing ${trackLabel}`, err.message);
            ...
            if (success) {
                interaction.editReply({
                    content: `Added to queue: ${trackLabel}`,
                    components: [],
                });
